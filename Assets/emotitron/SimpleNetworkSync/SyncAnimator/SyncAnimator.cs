...
namespace emotitron.Networking
{
		...
		, IOnCaptureState
	{
		...
		protected override void Reset()
		{
			...
			_applyOrder = ApplyOrderConstants.ANIMATOR;
		...
		{
			transform.GetNestedComponentsInChildren(foundAnimators);
			transform.GetNestedComponentsInChildren(foundSyncs);
		...
		{
			var passThrus = frame.passThrus;
			SerializationFlags flags = (passThrus.Count == 0) ? SerializationFlags.None : SerializationFlags.HasChanged;
			...
			for (int layer = 0; layer < count; ++layer)
			{
				...
				if (isKeyframe || stateHasChange)
				{
					...
					else
					{
						...
						buffer.WriteNorm(normaltimes[layer], ref bitposition, (int)normalizedTimeCompress);
					}
			...
			for (int layer = 0; layer < count; ++layer)
			{
				...
				if (stateHasChange)
				{
					...
					else
					{
						...
						statehashes[layer] = hash;
						normaltimes[layer] = buffer.ReadNorm(ref bitposition, (int)normalizedTimeCompress);
					}
				}
		...
		public override bool OnSnapshot(int newTargetFrameId)
		{
			...
			bool wasValid = !ReferenceEquals(targFrame, null) && targFrame.content != 0; // netObj.validFrames.Get(targF.frameId);

			if (wasValid)
				ApplyFrame(targFrame);
		...
		///  UNTESTED
		protected override FrameContents InterpolateFrame(Frame targFrame, Frame startFrame, Frame endFrame, float t)
		{
		...
		{
			ExtrapolateParams(pre1Frame, snapFrame, targFrame);
			ExtrapolateState();
			return FrameContents.Complete;
		}
