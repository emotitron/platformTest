...
namespace emotitron.Networking
{
		...
		, IVitalMonitor
	{
		...
		public override void UpdateGraphics(Vital vital)
		{
			...
			if (barSprite)
				barSprite.size = new Vector2((val / vitalDef.MaxValue) * widthMultiplier, barSprite.size.y);
			...
			if (UIImage != null)
			{
				...
				if (UIImage.type == Image.Type.Filled && UIImage.sprite != null)
					UIImage.fillAmount = (val / fullval) * widthMultiplier;
				else
					UIImage.rectTransform.localScale = new Vector3(
						(val / fullval) * widthMultiplier,
