...
namespace emotitron.Utilities.GUIUtilities
{
	...
	public abstract class MaskLogic
	{
		...
		protected const string HELP_URL = "https://docs.google.com/document/d/1ySmkOBsL0qJnIk7iN9lbXPlfmYTGkN7JFgKDBdqj9e8/edit#bookmark=id.itise18729xl";
		...
		public void DrawGUI(SerializedProperty sp)
		{
			...
			for (int i = 0; i < enumNames.Length; ++i)
			{
				...
				if (operation.intValue != (int)Operator.EQUALS)
				{
					...
					{
						if (i == 1)
							EditorGUI.LabelField(new Rect(r) { width = lwidth }, "If " + EnumTypeName + " is ", (GUIStyle)"RightLabel");
					}
					else if (set)
					{
						string conj = operation.intValue == (int)Operator.AND ? "and is " : "or is ";
						EditorGUI.LabelField(new Rect(r) { width = lwidth }, (i == firstSetBit ? "If " + EnumTypeName + " is " : conj) + (not ? "not " : ""), (GUIStyle)"RightLabel");
