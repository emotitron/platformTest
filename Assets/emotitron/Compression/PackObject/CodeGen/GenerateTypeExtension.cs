...
namespace emotitron.Compression.Internal
{
	public static class GenerateTypeExt
	{
		...
		public static StringBuilder GeneratePackCode(this Type objType, TypeInfo objTypeInfo, PackObjectAttribute pObjAttr)
		{
			...
			sb._("using emotitron.Utilities;").EOL();
			sb._("using emotitron.Utilities.Networking;").EOL();
			...
			/// Namespace
			{
				sb._("namespace emotitron.Compression.Internal").EOL();
				sb._("{").EOL();
		...
		private static void GenerateFields(Type pObjType)
		{
			...
			for (int i = 0; i < cnt; ++i)
			{
				...
				var packAttrs = fieldinfo.GetCustomAttributes(typeof(PackBaseAttribute), false);

				if (defaultInclusion == DefaultPackInclusion.Explicit && packAttrs.Length == 0)
					continue;

				PackBaseAttribute packAttr; // = (objType.GetCustomAttributes(typeof(PackBaseAttribute), false)[0] as PackBaseAttribute);
				...
				{
					packAttr = new PackAttribute();
					generatePackAttribute = true;
				}
				else
				{
					packAttr = packAttrs[0] as PackBaseAttribute;
					generatePackAttribute = false;
				}

				if (!isPublic)
				{
					Debug.LogWarning("Field <b>" + fname + "</b> in <b>" + objTypeName + "</b> has " + packAttr.GetType().Name + ", but is not public. Will be ignored. Set to public if you would like it included in packing.");
					continue;
				}
				...
				if (!TypeIsHandled(ftype, packAttr))
				{
					...
					if (!isNestedPackObj)
					{
						...
						Debug.LogWarning("<b>[" + attrname.Replace("Attribute", "") + "]</b> does not support packing of type <b>"
							+ ftypename + "</b> on field <b>" + objTypeName + "." + fname + "</b> . Will be ignored.");
					...
					{
						Debug.LogWarning("<b>" + objTypeName + "</b> contains a nested loop with field <b>" + ftype + " " + fname + "</b>. Will not be included in serialization.");
						continue;
					}
		...
		private static void Generate_Factory(FieldInfo fInfo, StringBuilder sb, string objName, string fname, string ftypename, string ffulltype, bool isNest, Type pObjType, PackBaseAttribute fAttr)
		{
		...
		private static void Generate_FieldFrame(FieldInfo fInfo, StringBuilder sb, string fname, string ftypename, string ffulltype, bool isNest, PackBaseAttribute fAttr)
		{
			if (isNest)
		...
		private static bool IsSyncAsTrigger(PackObjectAttribute pObjAttr, PackBaseAttribute fAttr)
		{
		...
		private static void Generate_Capture(FieldInfo fInfo, StringBuilder sb, string objName, string fname, string ftypename, string ffulltype, bool isNest, Type pObjType, PackObjectAttribute pObjAttr, PackBaseAttribute fAttr)
		{
		...
		private static void Generate_CopyTo(FieldInfo fInfo, StringBuilder sb, string objName, string fname, string ftypename, string ffulltype, bool isNest, Type pObjType, PackBaseAttribute fAttr)
		{
			string s = pObjType.IsValueType ? "(*(" + objName + "*)src)" : "s";
		...
		/// Snapshot
		private static void Generate_Snapshot(FieldInfo fInfo, StringBuilder sb, string objName, string fname, string ftypename, string ffulltype, bool inNest, Type pObjType, PackBaseAttribute fAttr)
		{
			string t = pObjType.IsValueType ? "(*(" + objName + "*)trg)" : "t";
		...
		private static void Generate_ApplyToObj(FieldInfo fInfo, StringBuilder sb, string objName, string fname, string ftypename, string ffulltype, bool inNest, Type pObjType, PackObjectAttribute pObjAttr, PackBaseAttribute fAttr)
		{
			string t = pObjType.IsValueType ? "(*(" + objName + "*)trg)" : "t";
		...
		/// Lerp Frames to Obj
		private static void Generate_InterpToObj(FieldInfo fInfo, StringBuilder sb, string objName, string fname, string ftypename, string ffulltype, bool inNest, Type pObjType, PackBaseAttribute fAttr)
		{
		...
		private static void Generate_InterpToFrame(FieldInfo fInfo, StringBuilder sb, string objName, string fname, string ftypename, string ffulltype, bool inNest, Type pObjType, PackBaseAttribute fAttr)
		{
			//if (!fAttr.interpolate)
			//	return;
		...
		private static void Generate_CopyFrameToFrame(FieldInfo fInfo, StringBuilder sb, string objName, string fname, string ftypename, string ffulltype, bool isNest, Type pObjType, PackBaseAttribute fAttr)
		{
			string copyfield = fAttr.GetCopyCodeGen(fInfo.FieldType, fname, "s", "t");
		...
		private static void Generate_FieldInit(FieldInfo fInfo, StringBuilder sb, string fname, string ftypename, string ffulltype, string genericarg, string primitive, bool isNest, bool generateAttr, PackBaseAttribute fAttr)
		{
			Type ftype = fInfo.FieldType;
			...
			else
			{
				if (generateAttr)
					...
					sb.____(fAttr.GetType().Name, " ", fname, "PackAttr = (", fAttr.GetType().Name, ")(typeof(", fullDeclaring)._(").GetField(\"", fname, "\").GetCustomAttributes(typeof(PackBaseAttribute), false)[0] as ", fAttr.GetType().Name, ");").EOL();
		...
		private static void Generate_FieldUnpack(FieldInfo fInfo, StringBuilder sb, string fname, string ftypename, string ffulltype, bool isNestedPackObj, string enumUnderCast, string enumTypeCast, PackBaseAttribute fAttr)
		{
			if (isNestedPackObj)
				sb.____("flags |= ")
		...
		private static bool TypeIsHandled(Type fieldType, PackBaseAttribute packAttr)
		{
			if (fieldType.IsEnum)
		...
		public static bool CheckForNestedLoop(Type type, Type checkIn)
		{
			...
			foreach (var field in fields)
			{
				...
				if (field.FieldType.GetCustomAttributes(typeof(PackBaseAttribute), false).Length != 0)
